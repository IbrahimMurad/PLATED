...
class CurrentSemesterManagerTestCase(TestCase):
    ...
    def test_current_semester_queryset(self):
        ...
        current_semesters = Semester.current.filter(curriculum=self.curriculum)
...
class SemesterCurriculumRelationTestCase(TestCase):
    ...
    def test_semester_curriculum_is_Curriculum_instance(self):
        semester = Semester.objects.create(curriculum=self.curriculum)
        self.assertIsInstance(semester.curriculum, Curriculum)

    def test_semester_curriculum_is_deleted(self):
    ...
    def test_path_curriculum_is_updated(self):
        ...
        self.assertEqual(
            semester.curriculum.name, "Egyptian national curriculum updated"
        )

    def test_path_curriculum_is_updated_with_new_instance(self):
        semester = Semester.objects.create(curriculum=self.curriculum)
        new_curriculum = Curriculum.objects.create(name="IGCSE", type="INTERNATIONAL")
    ...
    def test_get_semester_by_curriculum(self):
        s1 = Semester.objects.create(curriculum=self.curriculum)
        s2 = Semester.objects.create(
            curriculum=self.curriculum,
            starts_at=date(2024, 2, 1),
            ends_at=date(2024, 5, 31),
        )
        queryset = Semester.objects.filter(curriculum=self.curriculum)
