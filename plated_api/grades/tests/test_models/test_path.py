...
class PathTestCase(TestCase):
    ...
    def test_Path_with_invalide_language_choice(self) -> None:
    ...
    def test_display_value_of_Path_type(self) -> None:
        c: Curriculum = Curriculum.objects.create(name="Egyptian national curriculum")
        p: Path = Path.objects.create(
    ...
    def test_Path_name_is_required(self) -> None:
        c: Curriculum = Curriculum.objects.create(name="Egyptian national curriculum")
        with self.assertRaises(ValidationError):
            Path.objects.create(curriculum=c)

    def test_Path_curriculum_is_required(self) -> None:
        with self.assertRaises(ValidationError):
            Path.objects.create(name="Arabic path")

    def test_Path_main_language_is_required(self) -> None:
        with self.assertRaises(ValidationError):
            c: Curriculum = Curriculum.objects.create(
                name="Egyptian national curriculum"
            )
            Path.objects.create(  # type: ignore
                name="Egyptian national Path", curriculum=c, main_language=None
            )

    def test_Path_language_default(self) -> None:
        # not passing a type
        c: Curriculum = Curriculum.objects.create(name="Egyptian national curriculum")
        p: Path = Path.objects.create(name="Arabic path", curriculum=c)
        self.assertEqual(p.main_language, "ENGLISH")
        self.assertEqual(p.get_main_language_display(), "English")

    def test_Path_id_is_uuid(self) -> None:
        c: Curriculum = Curriculum.objects.create(name="Egyptian national curriculum")
        p: Path = Path.objects.create(name="Arabic path", curriculum=c)
        self.assertIsNotNone(p.id)
        self.assertTrue(isinstance(p.id, UUID))

    def test_Path_created_at(self) -> None:
        c: Curriculum = Curriculum.objects.create(name="Egyptian national curriculum")
        p: Path = Path.objects.create(name="Arabic path", curriculum=c)
        self.assertIsNotNone(p.created_at)
        self.assertIsInstance(p.created_at, datetime)

    def test_Path_updated_at(self) -> None:
        c: Curriculum = Curriculum.objects.create(name="Egyptian national curriculum")
        p: Path = Path.objects.create(name="Arabic path", curriculum=c)
        first_updated_value: datetime = p.updated_at
    ...
    def test_Path_get_by_name(self) -> None:
        c: Curriculum = Curriculum.objects.create(name="Egyptian national curriculum")
        p: Path = Path.objects.create(name="Arabic path", curriculum=c)
...
class PathCurriculumRelationTestCase(TestCase):
    ...
    def test_path_curriculum_is_Curriculum_instance(self) -> None:
        c: Curriculum = Curriculum.objects.create(name="Egyptian national curriculum")
        p: Path = Path.objects.create(name="Arabic path", curriculum=c)
    ...
    def test_path_curriculum_is_required(self) -> None:
        with self.assertRaises(ValidationError):
            Path.objects.create(name="Arabic path", curriculum=None)  # type: ignore

    def test_path_curriculum_is_deleted(self) -> None:
        c: Curriculum = Curriculum.objects.create(name="Egyptian national curriculum")
    ...
    def test_path_curriculum_is_updated_with_new_instance(self) -> None:
        c1: Curriculum = Curriculum.objects.create(name="Egyptian national curriculum")
        c2: Curriculum = Curriculum.objects.create(name="IGCSE curriculum")
        p: Path = Path.objects.create(name="Arabic path", curriculum=c1)
        p.curriculum = c2
        p.save()
        p.refresh_from_db()
        self.assertEqual(p.curriculum.name, "IGCSE curriculum")

    def test_get_path_by_curriculum(self) -> None:
        c: Curriculum = Curriculum.objects.create(name="Egyptian national curriculum")
        p1: Path = Path.objects.create(
            name="Arabic path", curriculum=c, main_language="ARABIC"
        )
        p2: Path = Path.objects.create(name="English path", curriculum=c)
        queryset: QuerySet = Path.objects.filter(curriculum=c)
    ...
    def test_get_path_by_curriculum_name(self) -> None:
        ...
        p2: Path = Path.objects.create(name="English path", curriculum=c)
        queryset: QuerySet = Path.objects.filter(
            curriculum__name="Egyptian national curriculum"
        )
        self.assertEqual(queryset.count(), 2)
        self.assertIn(p1, queryset)
        self.assertIn(p2, queryset)
